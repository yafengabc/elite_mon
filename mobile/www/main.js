...
async function poll(){
    // Mobile only: the API lives on the PC, so its address is prefixed here
    // (app.js). On the web panel apiBase() is empty and the path stays relative.
    const base = apiBase();
    if(!base){ showSetup(); return; }   // nothing configured yet: ask, do not poll
    try{
        const res = await fetch(base + "/api/status", {cache:"no-store"});
        if(!res.ok){ throw new Error("HTTP " + res.status); } // awaiting without checking the status fails silently
        render(await res.json());
        setServerStatus("ok", base);
    }catch(e){
        console.error(e);
        setHTML("loginfo", '<span class="warn">' + T("panel.connect_failed", esc(e.message)) + '</span>');
        setServerStatus("err", base);
...
async function init(){
    ...
    try{
        const res = await fetch(apiBase() + "/api/i18n", {cache:"no-store"});
        if(res.ok){
